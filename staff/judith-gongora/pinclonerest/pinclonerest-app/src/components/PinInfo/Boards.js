...
class Boards extends Component {
    ...
    render() {
        return <section className="container__boards-pinned">
            ...
            {this.state.search.map(board => <li className='list__boards-item' id={board.id} onClick={() => this.props.handleSelectBoard(board)} > <span>{board.title} </span><button className="button-list-select" type="submit" >Save</button></li> )}
            </ul>
